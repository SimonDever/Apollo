...
import { ESSENTIAL_FIELDS } from '../models/metadata-fields';
...
export class SearchService {
	...
	constructor(
		...
		this.config$.pipe(map((config) => this.config = config)).subscribe();
	}
		...
		this.movieDb.search.getMulti(
			...
			function (data) {
				...
				try {
					...
					this.store.dispatch(
						new LibraryActions.ShowMetadataResults({ results: response })
					);
				}
			}.bind(this),
			function (data) {
				...
				this.store.dispatch(
					new LibraryActions.ShowMetadataResults({
						results: {
							page: 1,
							results: [],
							total_pages: 1,
		...
		for (const prop in entry) {
			if (prop !== 'sort_order' && entry[prop] != null) {
				if (Array.isArray(entry[prop])) {
					console.log(
						'searchService.cleanArrays :: prop is array = ',
						Array.isArray(entry[prop])
					);
					entry[prop] = entry[prop].map((e) => e.name).join(', ');
				} else if (typeof entry[prop] === 'object') {
					console.log(
						'searchService.cleanArrays :: typeof entry[prop] is object'
					);
					entry[prop] = entry[prop].name;
				}
			} else {
				console.debug(
					'searchService.cleanArrays :: entry[prop] is null, prop is ' + prop
				);
			}
		}
	...
	getFirstResult(entry: any): Observable<any> {
		...
		return new Observable((subscriber) => {
			...
			mergeMap((options: any) => {
				...
				return new Observable((subscriber) => {
					this.movieDb.movies.getById(
						{ id: options[0].id },
						(data) => {
							let parsedData: any = JSON.parse(data);
							console.log('searchService :: getFirstResult :: output', parsedData);
	...
	details(id: string, media_type: string): any {
		...
		resource.getById(
			...
			function (data) {
				...
				this.store.dispatch(
					new LibraryActions.ShowMetadataDetailsResults({ details })
				);
			}.bind(this),
			function (data) {
				...
				console.error(data);
				this.store.dispatch(
					new LibraryActions.ShowMetadataDetailsResults({
